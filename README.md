...
Windows 桌面应用启动器。启动后会读取当前 Windows 用户桌面上的 `.lnk` 快捷方式和文件夹，也支持用户手动添加任意应用、快捷方式、文件或文件夹。用户可以在页面中配置多字符快捷键；工具会在后台监听键盘，输入 `/快捷键` 后打开对应应用或文件夹。
...
快捷键、手动添加应用和隐藏应用配置也保存到运行者自己的用户数据目录，不会读取开发者电脑上的配置。
