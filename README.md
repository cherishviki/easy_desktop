...
打包产物已进行以下优化（解压后约 245 MB）：

- 关闭生产构建 sourcemap
- 开启 ASAR 压缩
- afterPack 脚本自动清理：仅保留 zh-CN 和 zh-TW 语言包、移除 Chromium 许可证文件、移除 ffmpeg.dll、移除 GPU 渲染 DLL
- 应用仅需软件渲染模式，禁用了 GPU 加速以减少依赖

## 后台内存优化

应用隐藏到系统托盘时会销毁渲染窗口（释放约 150 MB 内存），键盘快捷键监听仍在后台运行。点击托盘图标或菜单"显示窗口"时自动重建窗口并恢复之前的位置和大小。